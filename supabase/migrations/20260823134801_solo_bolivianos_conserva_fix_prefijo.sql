-- Vuelve a poner el orden correcto de upper()/filtro en el prefijo.
--
-- La migración anterior restauró admin_create_project desde el archivo VIEJO
-- (multi_proyecto) en vez del que ya traía el arreglo (fix_prefijo_proyecto),
-- así que reintrodujo un error ya corregido: filtrar [^A-Z0-9] antes de upper()
-- se come las minúsculas del prefijo derivado del slug y TODAS las
-- urbanizaciones terminan con el prefijo 'PRY' — y la segunda choca con
-- PREFIX_TAKEN y no se puede crear.
...
begin
  ...
  -- upper() ANTES de filtrar: al revés, el filtro se comía las minúsculas del
  -- prefijo derivado y todos terminaban en 'PRY'.
