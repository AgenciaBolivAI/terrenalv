-- Abonar a capital regenera el cronograma: también empareja y renumera.
create or replace function public.admin_register_cuota_payment(
  p_reservation_id uuid, p_amount numeric, p_paid_on date default null,
  p_provider payment_provider_kind default 'efectivo', p_reference text default null,
  p_note text default null, p_treasury_account_id uuid default null,
  p_currency character default 'BOB', p_exchange_rate numeric default null,
  p_destino text default null, p_recalculo text default null)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if v_destino = 'cuota' then
    ...
    if v_nuevo <= 0.01 then
      ...
      -- El plazo: 'plazo' = conservar la cuota de antes y acortar (el plazo
      -- que más se le acerca); 'cuota' = conservar la cantidad de cuotas.
      -- En los dos casos la cuota final la empareja el sistema.
      ...
      -- Cuotas SIEMPRE parejas para el plazo que quedó: francesa con interés,
      -- reparto igual sin él. Solo la última varía por centavos.
      if v_tasa > 0 then
        v_cuota := round(v_nuevo * (v_tasa/100) / (1 - power(1 + v_tasa/100, -v_meses)), 2);
      else
        v_cuota := round(v_nuevo / v_meses, 2);
        if v_cuota <= 0 or round(v_nuevo - v_cuota * (v_meses - 1), 2) <= 0 then
      ...
      -- Las cuotas del comprador van 1..n; las anuladas, detrás. Este era el
      -- único camino que regeneraba sin renumerar.
      perform private.renumerar_cuotas(v_plan.id);
