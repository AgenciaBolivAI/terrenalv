-- La venta de oficina dice cómo se pagó.
--
-- Estaba escrito 'manual_qr' a mano, así que TODA venta registrada desde el
-- mostrador quedaba como QR aunque hubiera entrado en efectivo. Con eso el
-- desglose por vía miente desde el origen: el efectivo que hay que arquear
-- aparece como plata que debería estar en el extracto del banco.
--
-- Se reemplaza en vez de sobrecargar: dos firmas con el mismo nombre dejan a
-- PostgREST sin saber cuál llamar.
drop function if exists public.admin_reserve_offline(uuid, text, text, text, text, int, text);

create function public.admin_reserve_offline(
  p_lot_id uuid,
  p_full_name text,
  p_ci text,
  p_phone text,
...
begin
  ...
  perform private.audit('team', v_actor, null, 'lot.reserved_offline', v_lot.project_id,
    ...
    null, jsonb_build_object(
      ...
      'forma_de_pago', coalesce(p_provider, 'manual_qr'),
...
revoke execute on function public.admin_reserve_offline(
  uuid, text, text, text, text, integer, text, public.payment_provider_kind) from public, anon;
