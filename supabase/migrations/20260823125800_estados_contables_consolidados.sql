-- Estados contables de la empresa, no de una urbanización.
--
-- Terrenalv S.R.L. es UNA sociedad con varias urbanizaciones: su balance y su
-- estado de resultados son los de la empresa entera. Estos reportes exigían un
-- proyecto, así que el "balance" que se veía era el de un proyecto y nunca el
-- de la sociedad — que es el que se presenta y el que firma el contador.
--
-- Ahora p_project_id nulo significa TODAS. Pasando una urbanización se sigue
-- obteniendo la suya, que es lo que sirve para saber si un proyecto puntual da
-- ganancia.
--
-- Los montos ya vienen en bolivianos del libro diario (amount_bob), así que
-- consolidar no mezcla monedas.

create or replace function public.rep_sumas_y_saldos(
  p_project_id uuid, p_desde date default null, p_hasta date default null)
returns table(cuenta text, cuenta_nombre text, tipo text, sort_order integer,
...
create or replace function public.rep_estado_resultados(
  p_project_id uuid, p_desde date default null, p_hasta date default null)
returns table(seccion text, cuenta text, cuenta_nombre text, sort_order integer, monto numeric)
language sql
stable
set search_path to 'public', 'extensions'
as $fn$
...
create or replace function public.rep_balance_general(
  p_project_id uuid, p_hasta date default null)
returns table(seccion text, cuenta text, cuenta_nombre text, sort_order integer, monto numeric)
language sql
stable
set search_path to 'public', 'extensions'
