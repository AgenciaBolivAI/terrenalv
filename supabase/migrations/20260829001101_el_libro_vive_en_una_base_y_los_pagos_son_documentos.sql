-- EL CUERPO DEL LIBRO SE MUDA A `private.libro_base`.
--
-- `v_libro_diario` pasa a ser una sola línea: `select * from libro_base where
-- private.ve_contabilidad()`. Dos razones, las dos aprendidas a golpes hoy:
--
--   · Cada vez que hubo que tocar una rama había que volver a escribir las
--     veinticinco, porque `create or replace view` exige la misma lista de
--     columnas y la puerta de permisos estaba metida en el mismo objeto.
--   · Al intentar envolverla sin reescribirla caí DOS VECES en definir una
--     vista en términos de sí misma («infinite recursion detected in rules»).
--     Con el cuerpo en otro objeto, el envoltorio nunca más se toca.
--
-- Y de paso cambian dos cosas de fondo:
--
--   · Los pagos a proveedor ya no salen de `expenses.pagado_el` sino de la
--     tabla `pagos_a_proveedor`, así que un pago parcial asienta lo que se
--     pagó y la deuda queda por el resto.
--   · El sueldo devengado (forma_pago = 'planilla') acredita 2.01.07.010
--     SUELDOS POR PAGAR en vez de la caja: el gasto es del mes trabajado y la
--     plata sale cuando se paga.
create or replace view private.libro_base as
with base_venta as (
  select r.*,
         case
...
union all
-- Egreso: contra qué. Al contado sale de la caja; a crédito se le debe al
-- proveedor; de un fondo, descarga el fondo; de planilla, se le debe al personal.
select e.project_id, e.incurred_on,
       coalesce(e.numero, 'EGR-' || left(replace(e.id::text, '-', ''), 10)),
       coalesce(ec.nombre || ' — ', '') || e.description || coalesce(' — ' || coalesce(c.name, e.supplier), ''),
       case e.forma_pago
         when 'credito' then '2.01.04.010'::text
         when 'planilla' then '2.01.07.010'::text
 ...
 where e.deleted_at is null
union all
-- Compra de activo fijo: entra al activo (solo si no nació de un egreso).
...
union all
-- Pago a proveedor (o al personal): cancela la deuda, en parte o del todo.
select pg.project_id, pg.fecha, pg.numero,
       case when pg.expense_id is not null
            then case when e.forma_pago = 'planilla' then 'Pago de sueldo — ' else 'Pago a proveedor — ' end
                 || e.description || coalesce(' — ' || coalesce(c.name, e.supplier), '')
            else 'Pago del activo fijo — ' || av.nombre end,
       case when e.forma_pago = 'planilla' then '2.01.07.010'::text else '2.01.04.010'::text end,
       pg.monto, 0::numeric,
       pg.id, 'pago_proveedor'::text,
       null::text, null::text,
       coalesce(e.centro_costo_id, av.centro_costo_id), cc.nombre,
       coalesce(e.titular, av.titular), coalesce(e.titular_nombre, av.titular_nombre),
       pg.created_at, pg.updated_at, pg.created_by, pr.full_name,
       'BOB'::text, 1::numeric, pg.monto
  from public.pagos_a_proveedor pg
  left join public.expenses e on e.id = pg.expense_id
  left join public.fixed_assets av on av.id = pg.activo_id
  left join public.contacts c on c.id = e.contact_id
  left join public.centros_costo cc on cc.id = coalesce(e.centro_costo_id, av.centro_costo_id)
...
select pg.project_id, pg.fecha, pg.numero,
  ...
  left join public.fixed_assets av on av.id = pg.activo_id
  left join public.contacts c on c.id = e.contact_id
  left join public.treasury_accounts t on t.id = pg.treasury_account_id
  left join public.centros_costo cc on cc.id = coalesce(e.centro_costo_id, av.centro_costo_id)
  left join public.profiles pr on pr.id = pg.created_by
 where pg.deleted_at is null
union all
...
alter view private.libro_base set (security_invoker = true);
grant select on private.libro_base to authenticated;

-- Y la vista pública queda en una línea, para siempre.
create or replace view public.v_libro_diario as
select * from private.libro_base where private.ve_contabilidad();

alter view public.v_libro_diario set (security_invoker = true);

comment on view public.v_libro_diario is
  'El libro diario. El cuerpo vive en private.libro_base; acá solo se le pone '
  'la puerta de permisos. Para agregar o cambiar una rama se toca libro_base, '
  'no esta vista.';
