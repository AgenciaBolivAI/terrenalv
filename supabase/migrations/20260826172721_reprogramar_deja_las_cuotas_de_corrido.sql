...
begin
  ...
  -- Recién ahora se numera lo que el comprador va a leer: 1, 2, 3… Las
  -- anuladas quedan guardadas, detrás, porque son el historial de lo que se
  -- había pactado antes — pero no le ocupan un número a su plan.
...
-- Los planes que ya quedaron salteados por el error anterior.
do $$
declare r record;
begin
  for r in
    select p.id from public.installment_plans p
     where exists (select 1 from public.installments i
                    where i.plan_id = p.id and i.status = 'anulada')
  loop
    perform private.renumerar_cuotas(r.id);
  end loop;
end $$;
