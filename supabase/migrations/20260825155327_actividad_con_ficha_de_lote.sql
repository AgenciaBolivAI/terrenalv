-- La ficha del lote en el perfil del cliente necesita el LOTE de verdad:
-- superficie, frente y fondo, esquina, estado físico y precio de lista — más
-- el desglose de la plata (seña, lo de antes, lo de acá). Columnas al final.
...
select r.buyer_ci_normalized as ci_norm,
       ...
       l.frontage_m,
       l.depth_m,
       l.is_corner as es_esquina,
       l.status::text as lot_status,
       public.lot_price(l.id) as precio_lista,
       v.cobrado_aqui,
       v.abonado_migrado,
       v.deuda_migrada,
       v.sena_pagada,
       v.sena_forma,
       v.traspaso_de_tracking,
       v.traspaso_de_comprador,
       r.client_meta->'traspasada_a'->>'comprador' as cedida_a_comprador,
       r.buyer_full_name,
       r.buyer_phone
